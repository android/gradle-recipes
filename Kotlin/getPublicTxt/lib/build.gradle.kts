...
abstract class PublicResourcesValidatorTask : DefaultTask() {
    ...
    @get:InputFile abstract val publicAndroidResources: RegularFileProperty

    @get:InputFile abstract val expectedPublicResources: RegularFileProperty

    @get:OutputDirectory abstract val fakeOutput: DirectoryProperty
...
androidComponents {
    onVariants { variant ->
        ...
        project.tasks.register<PublicResourcesValidatorTask>(
            "validate${capitalizedName}PublicResources") {
                publicAndroidResources.set(
                    variant.artifacts.get(SingleArtifact.PUBLIC_ANDROID_RESOURCES_LIST))
                expectedPublicResources.set(
                    project.file("src/test/expectedApi/public-resources.txt"))
                fakeOutput.set(
                    project.layout.buildDirectory.dir(
                        "intermediates/PublicResourcesValidatorTask/$name"))
            }
    }
}
